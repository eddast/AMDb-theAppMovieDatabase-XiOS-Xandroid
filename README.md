...
### Known Limitations
I thought it would be relatively easy renaming namespaces and project files of a project in Visual Studio and therefore I left that for last. Little did I know. After hours of googling I deem this task impossible given the time I have before the project is due. Therefore my android project of week two has the unfortunate name of "IOSWeek1". Also, the app can crash at random if there is too much load on the server containing the movie database due to malformed response from server, but since teacher stated that this would not be an issue in grading these cases are not explicitly handled due to limited time for the project.
